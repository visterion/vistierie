...
Verify with `git config user.name` / `git config user.email` before the first commit on a new clone — the system default (`vivu <owner@example.com>`) shows up as a different person on GitHub. If a commit slipped through under the wrong identity, rewrite with:
