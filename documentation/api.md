...
| `tool_choice` | object | no | Forces (or steers) tool use, e.g. `{"type": "tool", "name": "submit_mailings"}`. Omitting it lets the model decide. `{"type": "tool", "name": ...}` is the shape that requests a forced tool call — the matching entry in `tools` is the one the model must use; whether the provider can actually guarantee this is provider-specific (see providers.md). When forced, the answer arrives in `content_blocks` as a `tool_use` block instead of in `text` |
